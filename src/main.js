...
var schema = treo.schema()
	.version(1)
		...
		.addIndex('thumb','thumb')

var db = treo('folders', schema);
...
function updateFolder(folder = ''){
	...
	images.index('byFolder').get(folder, function(err, img) {
		for(var i in img) $('#folder-content').append(
				$('<div style="margin: 0px 4px 2px 0;"><img src="http://i.imgur.com/'+img[i].thumb+'"/></div>')
					.click(function(e){
						copyTextToClipboard('http://i.imgur.com/'+img[i].filename);
					})
			)
	});
}
...
function a() {
	$('.post-image').each(function(i, elem){
		...
		$('<div class="folders" style="position: absolute; margin-top: 20px; padding: 1px 4px 0 2px; width:235px; text-align:center; color: #f2f2f2; left: '+($(elem).position().left-34)+'px; background-color: #2c2f34;">')
			.append(
				...
				$('<button class="btn-small" style="display: inline-block; background-color: #1bb76e; padding: 8px 10px 6px; font-weight: 600;">Add</button>')
					.click(function(e){
						...
						images.put({filename: elem.parentElement.id, name: name, folder: folder, thumb: thumb.join('.')}, function(err, image){});
...
$('<div style="margin: 0 auto; width: 900px"><div style="position: fixed; bottom: 0; width: 220px; margin-left: 728px; z-index: 100;"><div style="padding:10px; border-radius: 3px 3px 0 0; background-color: #2c2f34;"><h3>folders</h3></div><div id="folder-content" style="height: 200px; display: flex; flex-wrap: wrap; overflow-y: scroll; padding: 4px 4px 4px 10px; background-color: black;"></div><div style="background-color: #2c2f34; padding: 4px 2px 9px;"><input type="text" style="width: 75%; margin: 0 3px 0 0;" placeholder="Image URL" /><button class="btn-small" style="display: inline-block; background-color: #1bb76e; padding: 8px 10px 6px; font-weight: 600;">Add</button></div></div></div></div>')
	.insertAfter(document.body.lastElementChild);

$(document.body).keypress(function(e){if(e.keyCode === 113 && $('.folders').length === 0) a()})
updateFolder();

//images.index('byName').get('Fred', function(err, img) {});

/*
$('<div style="position: absolute; margin-top: 20px; padding: 0 20px 0 2px; width:172px; text-align:center; color: #f2f2f2; left: 60px; background-color: #2c2f34;">').append(
	$('<div class="icon-plus" style="display: inline-block;vertical-align: sub;padding: 0 3px 0 4px;"></div>').click(function(e){$(this).animate({left: "0px"}, function(){$(this).css('z-index','10')})});
)<input type="text" style="display: inline-block; width: 80%; margin: 2px 2px;"></div>*/